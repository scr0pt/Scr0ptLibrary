...
suspend fun outlookRegister(email: String, collection: MongoCollection<org.bson.Document>) {
    ...
    val outlookRegisterPageManager = PageManager(arrayListOf(
    ...
    outlookRegisterPageManager.run { response ->
        println(response)
...
class OutlookRegisterEnterEmailPage(
    ...
    override fun _action(driver: WebDriver): PageResponse {
        ...
        driver.findElWait(1000, 50000, "input#MemberName", jsoup = false)?.firstOrNull()?.sendKeys(email)
            ?: return PageResponse.NOT_FOUND_ELEMENT()
        driver.findElWait(1000, 50000, "#iSignupAction", jsoup = false)?.firstOrNull()?.click()
            ?: return PageResponse.NOT_FOUND_ELEMENT()
...
class OutlookRegisterEnterPasswordPage(
    ...
    override fun _action(driver: WebDriver): PageResponse {
        println(this::class.java.simpleName + ": action")
        driver.findElWait(1000, 50000, "input#PasswordInput", jsoup = false)?.firstOrNull()?.sendKeys(password)
            ?: return PageResponse.NOT_FOUND_ELEMENT()
        driver.findElWait(1000, 50000, "#iSignupAction", jsoup = false)?.firstOrNull()?.click()
            ?: return PageResponse.NOT_FOUND_ELEMENT()
        return PageResponse.WAITING_FOR_RESULT()
...
class OutlookRegisterEnterNamePage(
    ...
    override fun _action(driver: WebDriver): PageResponse {
        println(this::class.java.simpleName + ": action")
        driver.findElWait(1000, 50000, "input#FirstName", jsoup = false)?.firstOrNull()?.sendKeys(firstName)
            ?: return PageResponse.NOT_FOUND_ELEMENT()
        driver.findElWait(1000, 50000, "input#LastName", jsoup = false)?.firstOrNull()?.sendKeys(lastName)
            ?: return PageResponse.NOT_FOUND_ELEMENT()
        driver.findElWait(1000, 50000, "#iSignupAction", jsoup = false)?.firstOrNull()?.click()
            ?: return PageResponse.NOT_FOUND_ELEMENT()
        return PageResponse.WAITING_FOR_RESULT()
...
class OutlookRegisterEnterBirthdatePage(
    ...
    override fun _action(driver: WebDriver): PageResponse {
        ...
        val BirthYear = Select(driver.findElement(By.id("BirthYear")))
        BirthYear.selectByValue(RandomUtils.nextInt(1980, 2001).toString())//exclude 2001
        val BirthDay = Select(driver.findElement(By.id("BirthDay")))
        BirthDay.selectByIndex(RandomUtils.nextInt(1, BirthDay.options.size))//exclude Day value empty
        val BirthMonth = Select(driver.findElement(By.id("BirthMonth")))
